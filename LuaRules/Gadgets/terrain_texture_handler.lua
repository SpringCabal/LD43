...
function gadget:GameFrame()
	...
	for i = 1, #roads do
		local road = roads[i]
		local minx, minz = math.floor(road[1]/8)*8, math.floor(road[2]/8)*8
		local maxx, maxz = math.floor(road[3]/8)*8, math.floor(road[4]/8)*8
		for x = minx, maxx, 8 do
			for z = minz, maxz, 8 do
				drawingList[#drawingList+1] = {x = x, z = z, tex = 1, edge = false}
			end
		end
	end
	...
	for i = 1, #tooEarly do
		GG.Terrain_Texture_changeBlockList(tooEarly[i])
	end
end
...
local texturePool = {
	...
	[1] = {
		texture = "LuaRules/Images/road.png",
		size = 64,
		tile = BLOCK_SIZE/64,
	},
	[2] = {
		texture = "LuaRules/Images/underhouse.png",
		size = 64,
		tile = BLOCK_SIZE/64,
	},
	[3] = {
...
function gadget:DrawGenesis()
	...
	local updateCount = 1
	local maxUpdateCount = 10 --how much texture update for each gadget:DrawWorld() frame
